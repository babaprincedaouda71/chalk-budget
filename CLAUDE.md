...
- **Design moderne (refonte du 19/07/2026, à la demande de l'utilisateur — le skeuomorphisme est abandonné)** : dashboard = surface sombre ardoise (`board` #0F172A, cartes `bg-white/5 ring-white/10`, accents émeraude `chalkGreen` / rose `brick`) ; autres pages = clair épuré (`paper` #F5F7FA, texte `ink`). Police unique Inter. Les noms de jetons historiques (board, chalk, brick, paper, ink…) sont conservés mais pointent vers la palette moderne ; les classes `.notebook-lines`, `.notebook-margin`, `.wood-frame` sont neutralisées dans `globals.css`. Ne pas réintroduire l'ancien style craie/carnet.
